const createMutationVariables = (fields) => {
  ...
  const mappedMutationVariables = fields.map((field, key) => {
    ...
    const nameField = (nField) => {
      ...
      const prefixInput = isName && nField?.inputs?.find((input) => input.key === 'prefix');
      const firstInput = isName && nField?.inputs?.find((input) => input.key === 'first');
      const middleInput = isName && nField?.inputs?.find((input) => input.key === 'middle');
      const lastInput = isName && nField?.inputs?.find((input) => input.key === 'last');
      const suffixInput = isName && nField?.inputs?.find((input) => input.key === 'suffix');
    ...
    const emailField = (eField) => {
      ...
      const emailConfirmation = (isEmail && eField.emailConfirmEnabled && `, $${value}ConfirmationValue: String${required}`) || '';
